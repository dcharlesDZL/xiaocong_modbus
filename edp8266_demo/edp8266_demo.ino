...
void loop()
{
    ...
    if (RS485.available()>0)          //判断串口缓冲区是否接受到数据
    {
      ...
      if (len = 35)                   //如果从机返回的数据长度等于35字节，则存储该数据
      {
        for (int i=0; i<len; i++)
        {
          RS485A.UN2.slavePtr[40] = RS485.read();     //将串口缓冲区的数据按字节存储到slavePtr[]数组中
        }
        //MasterReceProc(0x01);
      }
...
/*
  ...
  可修改对应寄存器中参数的值
*/
MasterSetData(u8 devnum, u8 idhigh, u8 idlow, u8 setdatahigh, u8 setdatalow)
{
